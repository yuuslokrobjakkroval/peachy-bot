	...
	async run(client, ctx, args, color, emoji, language) {
		...
		try {
			...
			try {
				...
				console.error("Error saving user after deducting coins:", saveError);
				return ctx.sendMessage({
					embeds: [
						client
							.embed()
							.setColor(color.danger)
							.setDescription("An error occurred while saving your data."),
					],
				});
			}
						...
						const resultEmbed = client
							...
							.setDescription(
								generalMessages.title
									...
									.replace("%{mainRight}", emoji.mainRight) +
									`${guessNumberMessages.correct.description} ${client.utils.getRandomElement(
										congratulations,
			...
			console.error("Error in guessnumber command:", error);
			return ctx.sendMessage({
				embeds: [
					client
						.embed()
						.setColor(color.danger)
						.setDescription("An error occurred while fetching your data."),
				],
			});
		}
	}
