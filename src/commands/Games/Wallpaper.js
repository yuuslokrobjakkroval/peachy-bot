...
module.exports = class Wallpaper extends Command {
	constructor(client) {
		super(client, {
			name: "wallpaper",
			description: {
				content: "View wallpapers for PC or phone",
				examples: ["wallpaper"],
				usage: "wallpaper",
			},
			cooldown: 5,
	...
	async run(client, ctx, args, color, emoji, language) {
		const selectedCategory = args[0] || wallpaperCategories[0]; // Default to the first category
		const wallpaper = language.locales.get(language.defaultLocale)?.funMessage
			?.wallpaper;
		const selectedWallpapers = Wallpapers.filter(
