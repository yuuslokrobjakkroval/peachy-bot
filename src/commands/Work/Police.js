...
module.exports = class CatchThief extends Command {
  constructor(client) {
    super(client, {
      ...
      description: {
        content:
          "Police can catch thieves who have successfully robbed someone.",
  ...
  async run(client, ctx, args, color, emoji, language) {
    ...
    try {
      ...
      if (
        police.work.position.toLowerCase() !== "police" ||
        police.work.status !== "approved"
      ) {
        return await client.utils.sendErrorMessage(
          client,
          ctx,
      ...
      if (success) {
        ...
        const policeEmoji = [
          globalEmoji.position.peachPolice,
          globalEmoji.position.gomaPolice,
        ];
