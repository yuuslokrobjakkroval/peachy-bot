...
module.exports = class Rob extends Command {
  ...
  async run(client, ctx, args, color, emoji, language) {
    ...
    try {
      ...
      if (
        robber.work.position.toLowerCase() === "police" &&
        robber.work.status === "approved"
      ) {
        return await client.utils.sendErrorMessage(
          client,
          ctx,
      ...
      if (
        victim.work.position.toLowerCase() === "police" &&
        victim.work.status === "approved"
      ) {
        return await client.utils.sendErrorMessage(
          client,
          ctx,
      ...
      if (!isCooldownExpired) {
        ...
        const cooldownMessage = robMessages.cooldown
      ...
      if (success) {
        ...
        const robEmoji = [
          globalEmoji.position.peachRob,
          globalEmoji.position.gomaRob,
        ];
