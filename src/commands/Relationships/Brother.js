        ...
        try {
            ...
            if (action === 'add') {
                if (userData.relationship.brothers.length >= 4) {
                    return await client.utils.sendErrorMessage(client, ctx, relationshipMessages.error.bro.error.limitExceeded, color);
                }
                if (userData.relationship.brothers.some(bro => bro.id === userToModify)) {
                    return await client.utils.sendErrorMessage(client, ctx, relationshipMessages.error.bro.error.alreadyExists, color);
                }
                userData.relationship.brothers.push({ id: userToModify, name: targetUserData.username, xp: 0, level: 1 });
            } else if (action === 'remove') {
                if (!userData.relationship.brothers.some(bro => bro.id === userToModify)) {
                    return await client.utils.sendErrorMessage(client, ctx, relationshipMessages.error.bro.error.notFound, color);
                }
            ...
            await client.utils.sendSuccessMessage(client, ctx, `${relationshipMessages.success[action]} <@${userToModify}> as your brother.`, color);
        } catch (err) {
            console.error(err);
        }
    }
