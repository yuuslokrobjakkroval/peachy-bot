...
module.exports = class ApprovedJob extends Command {
    ...
    super(client, {
      ...
      options: [
        ...
        },
        {
          name: "status",
          description:
            'The status of the job application, either "approved" or "rejected".',
          type: 3,
          required: true,
        },
        {
          name: "reason",
          description:
            'The reason for rejection (required if status is "rejected").',
          type: 3,
          required: false,
        },
      ],
  ...
  async run(client, ctx, args, color, emoji, language) {
    ...
    const status = ctx.isInteraction
      ? ctx.interaction.options.getString("status")
      : args[1];
