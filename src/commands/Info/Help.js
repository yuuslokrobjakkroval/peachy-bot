...
module.exports = class Help extends Command {
    ...
    async run(client, ctx, args, color, emoji, language) {
        ...
        categories.forEach((category) => {
            const categoryCommands = commands.filter((cmd) => cmd.category.toLowerCase() === category.name.toLowerCase());
            categoryPages[category.name] = Math.ceil(categoryCommands.size / COMMANDS_PER_PAGE) || 1;
        });
        ...
        if (!args[0]) {
            const messageOptions = () => {
                ...
                const categoryOptions = categories.map((category) => {
                    const categoryCommandCount = commands.filter((cmd) => cmd.category.toLowerCase() === category.name.toLowerCase()).size;
                    return {
                ...
                const categorySelectMenu = new StringSelectMenuBuilder()
                    ...
                    .setPlaceholder(`${emoji.decoration?.main || '🎯'} Select categories to explore • ${totalCommands} commands available`)
                    .setMinValues(1)
                    .setMaxValues(Math.min(categories.length, 25))
                    .addOptions(categoryOptions);

                // Enhanced navigation buttons with modern styling
                const homeButton = new ButtonBuilder().setCustomId('home').setLabel('🏠 Command Hub').setStyle(ButtonStyle.Primary);
                ...
                const supportButton = new ButtonBuilder()
                    .setLabel('💬 Get Help')
                    .setStyle(ButtonStyle.Link)
                    .setURL(client.config.links.support || 'https://discord.gg/peachyganggg');
                ...
                const buttonRow = new ActionRowBuilder().addComponents(homeButton, quickTipButton, dashboardButton, supportButton);
                ...
                try {
                    ...
                    if (interaction.customId === 'quick_tips') {
                        const tipsEmbed = client
                            .embed()
                            .setColor(color.main)
                            .setTitle(`${emoji.decoration?.main || '💡'} Quick Tips & Shortcuts`)
                            .setDescription(
                    ...
                    if (interaction.customId.startsWith('prev_page_') || interaction.customId.startsWith('next_page_')) {
                        ...
                        } else if (action === 'next' && currentPages[category] < categoryPages[category]) {
                            currentPages[category]++;
                        }
                ...
                const similarCommands = allCommands
                    .filter(
                        (cmd) =>
                            ...
                            cmd.aliases.some((alias) => alias.toLowerCase().includes(args[0].toLowerCase())) ||
            ...
            const categoryInfo = categories.find((cat) => cat.name === command.category.toLowerCase());
            const categoryEmoji = categoryInfo
                ? emoji.help[categoryInfo.name.toLowerCase()]
                    ? emoji.help[categoryInfo.name.toLowerCase()]
                    : globalEmoji.help[categoryInfo.name.toLowerCase()]
                : '📁';
            const categoryDesc = categoryInfo ? categoryInfo.description : 'General commands';

            // Create modern command detail embed with enhanced styling
            const cooldownIcon = command.cooldown <= 3 ? '⚡' : command.cooldown <= 10 ? '⏱️' : '🕐';
            const difficultyLevel = command.cooldown <= 3 ? 'Beginner' : command.cooldown <= 10 ? 'Intermediate' : 'Advanced';

            const helpEmbed = client
                .embed()
                .setColor(color.main)
                .setTitle(`${categoryEmoji} ${client.utils.formatCapitalize(command.name)} Command ${cooldownIcon}`)
                .setDescription(
            ...
            const row = new ActionRowBuilder().addComponents(backButton, categoryButton);
    ...
    async showMultiCategoryView(
        ...
        selectedCategories.forEach((categoryName) => {
            const categoryCommands = commands.filter((cmd) => cmd.category.toLowerCase() === categoryName.toLowerCase());
            ...
            if (categoryCommands.size > 0) {
                ...
                const commandsList = Array.from(categoryCommands.values())
                    ...
                    .map((cmd, index) => {
                        const cooldownIcon = cmd.cooldown <= 3 ? '⚡' : cmd.cooldown <= 10 ? '⏱️' : '🕐';
        ...
        const categorySelectMenu = new StringSelectMenuBuilder()
            .setCustomId('category_select')
            .setPlaceholder(`🎯 ${selectedCategories.length} selected • Modify selection... ✨`)
            .setMinValues(1)
            .setMaxValues(Math.min(categories.length, 25))
            .addOptions(categoryOptions);

        // Navigation buttons
        const homeButton = new ButtonBuilder().setCustomId('home').setLabel('Home').setStyle(ButtonStyle.Primary).setEmoji('🏠');
        ...
        const selectRow = new ActionRowBuilder().addComponents(categorySelectMenu);
        const buttonRow = new ActionRowBuilder().addComponents(homeButton, clearButton);
    ...
    async showCategoryPage(
        ...
        const categoryCommands = commands.filter((cmd) => cmd.category.toLowerCase() === category.toLowerCase());
        ...
        const pageCommands = Array.from(categoryCommands.values()).slice(startIdx, endIdx);
        ...
        const categoryEmoji = categoryInfo
            ? emoji.help[categoryInfo.name.toLowerCase()]
                ...
                : globalEmoji.help[categoryInfo.name.toLowerCase()]
            : '📁';
        const categoryDesc = categoryInfo ? categoryInfo.description : 'Commands in this category';

        // Create enhanced embed for the current page
        const selectedEmbed = client
            .embed()
            .setColor(color.main)
            .setTitle(`${categoryEmoji} ${categoriesMessages[category.toLowerCase()] || client.utils.formatCapitalize(category)} Commands`)
            .setDescription(
                `✨ **${categoryDesc}**\n\n` +
        ...
        const categoryOptions = categories.map((cat) => {
            const categoryCommandCount = commands.filter((cmd) => cmd.category.toLowerCase() === cat.name.toLowerCase()).size;
            return {
                emoji: emoji.help[cat.name.toLowerCase()] ? emoji.help[cat.name.toLowerCase()] : globalEmoji.help[cat.name.toLowerCase()],
                label: categoriesMessages[cat.name.toLowerCase()] || client.utils.formatCapitalize(cat.name),
                description: `${categoryCommandCount} commands available`,
        ...
        const categorySelectMenu = new StringSelectMenuBuilder()
            .setCustomId('category_select')
            .setPlaceholder(
                `📚 Currently viewing: ${categoriesMessages[category.toLowerCase()] || client.utils.formatCapitalize(category)}`
            )
            .setMinValues(1)
            .setMaxValues(Math.min(categories.length, 25))
            .addOptions(categoryOptions);
        ...
        let buttonRow;
        if (totalPages > 1) {
            buttonRow = new ActionRowBuilder().addComponents(prevButton, pageIndicator, nextButton, homeButton);
        } else {
