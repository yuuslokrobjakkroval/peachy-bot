...
module.exports = class Birthday extends Command {
    ...
    async run(client, ctx, args, color, emoji, language) {
        const bdMessages = language.locales.get(language.defaultLocale)?.profileMessages?.bdMessages;
        ...
        switch (subCommand) {
            case 'help': {
                // Show help
                embed.addFields([
                    { name: '**Usage :**', value: `\`\`\`birthday <date || reset || help>\n\`\`\``, inline: false },
            ...
            case 'reset': {
                ...
                embed.setDescription(bdMessages?.resetSuccess || 'Your birthday and zodiac sign have been reset.');
            ...
            default: {
                ...
                if (!date) {
                    await client.utils.oops(client, ctx, bdMessages?.provideDate || 'Please provide a valid date for your birthday or a valid subcommand.', color);
                    return;
                }
                ...
                if (!parsedDate.isValid()) {
                    embed.setDescription(`${bdMessages?.invalidDate || 'The date must be in one of the supported formats:'}\n• DD-MM\n• MM-DD\n• MM/DD\n• MM-DD\n• DD/MM`);
                    await ctx.sendMessage({ embeds: [embed] });
                } else {
