		...
		try {
			...
			try {
				...
				try {
					...
					console.error("Error editing loading message:", editError);
					await this.handleError(ctx, loadingMessage);
				}
			} catch (fetchError) {
...
Please wait a moment while we craft your personalized stats display...`,
	...
	drawPremiumBadge(ctx, x, y, size, level, baseColor, label = "TEXT") {
		try {
			ctx.save();
			...
			for (let i = 0; i < 5; i++) {
				...
				if (
					...
					this.drawStar(
						...
						y,
						size,
						`rgba(${Number.parseInt(starColor.slice(1, 3), 16)}, ${Number.parseInt(
							starColor.slice(3, 5),
							16,
	...
	async drawStreamlinedLevel(
		...
		try {
			...
			context.closePath();
			context.clip();
			context.drawImage(userAvatar, avatarX, avatarY, avatarSize, avatarSize);
			context.restore();
			...
			if (decorationImage) {
				try {
					...
					const decorationSize = 190; // 10px border around avatar
					const decorationX = avatarX + (avatarSize - decorationSize) / 2;
					const decorationY = avatarY + (avatarSize - decorationSize) / 2;
					context.save();
					context.beginPath();
					context.arc(
						avatarX + avatarSize / 2,
			...
			context.textAlign = "center";
			context.fillText(
				`${client.utils.formatNumber(currentXP)} / ${client.utils.formatNumber(maxXP)} Text XP`,
				cardX + cardWidth / 2,
				progressBarY + progressBarHeight + 15,
			);
			context.fillText(
				`${client.utils.formatNumber(currentVoiceXP)} / ${client.utils.formatNumber(maxVoiceXP)} Voice XP`,
				cardX + cardWidth / 2,
