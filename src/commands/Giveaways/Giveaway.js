...
module.exports = class Giveaway extends Command {
    constructor(client) {
        super(client, {
            ...
            description: {
                ...
                examples: ['gstart "Win amazing coins!" 1000 1h 2', 'gstart "Weekly giveaway" 10000 12h 5'],
                usage: 'gstart <description> <prize> <duration> <winners> [image] [thumbnail] [autopay]',
            },
            category: 'giveaways',
            aliases: ['gs'],
            args: true,
    ...
    async run(client, ctx, args, color, emoji, language) {
        ...
        const description = ctx.isInteraction ? ctx.interaction.options.getString('description') : args[0];
        let prize = ctx.isInteraction ? ctx.interaction.options.getString('prize') : args[1];
        const durationStr = ctx.isInteraction ? ctx.interaction.options.getString('duration') : args[2];
        const winnersStr = ctx.isInteraction ? ctx.interaction.options.getInteger('winners') : args[3];
        const image = ctx.isInteraction ? ctx.interaction.options.getAttachment('image') : null;
        const thumbnail = ctx.isInteraction ? ctx.interaction.options.getAttachment('thumbnail') : null;
        const autoPay = ctx.isInteraction ? ctx.interaction.options.getString('autopay') : args[6];

        // Validate common parameters
        const winners = Number.parseInt(winnersStr, 10);
        const validationResult = await client.utils.validateCommonParams(ctx, client, color, durationStr, winners);
        ...
        if (typeof prize === 'string' && prize.match(/\d+[kmbtq]/i)) {
                ...
                return ctx.sendMessage({
                    embeds: [
                        client
                            ...
                            .setDescription('Invalid prize format. Please provide a valid number or multiplier (e.g., 1k, 1m).'),
                    ],
                    flags: MessageFlags.Ephemeral,
                });
            }
            ...
            if (isNaN(prize) || prize <= 0) {
                return ctx.sendMessage({
                    embeds: [client.embed().setColor(color.danger).setDescription('Prize must be a positive integer.')],
                    flags: MessageFlags.Ephemeral,
                });
            }
        }
        ...
        if (autoPay) {
            ...
            if (!(await client.utils.hasSpecialPermission(ctx.author.id))) {
                ...
                console.log(`Checking balance for user ${ctx.author.id}:`, getUser.balance.coin);
                ...
                try {
                    ...
                    console.error(`Error updating user balance for ${ctx.author.id}:`, error);
                    return ctx.isInteraction
                        ? await ctx.interaction.editReply({
                              content: '❌ Error updating your balance. Please try again later.',
                              flags: MessageFlags.Ephemeral,
                          })
                        : await ctx.editMessage({
                              content: '❌ Error updating your balance. Please try again later.',
        ...
        const giveawayEmbed = client
            .embed()
            .setColor(color.main)
            .setTitle(description ? `${description}` : `**${client.utils.formatNumber(prize)}** ${emoji.coin}`)
            .setDescription(
        ...
        if (!messageResult.success) {
            ...
            if (isAutopayEnabled && !(await client.utils.hasSpecialPermission(ctx.author.id))) {
                try {
                    const getUser = await client.utils.getUser({ userId: ctx.author.id });
                    getUser.balance.coin += prize * winners;
                    await getUser.save();
                } catch (error) {
                    console.error(`Error refunding user balance for ${ctx.author.id}:`, error);
                }
            }
        ...
        try {
            ...
            return ctx.channel.send({
                embeds: [client.embed().setColor(color.danger).setDescription('There was an error saving the giveaway. Please try again.')],
                flags: MessageFlags.Ephemeral,
            });
        }
    }
