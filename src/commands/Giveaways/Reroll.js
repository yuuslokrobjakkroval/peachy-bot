...
module.exports = class Reroll extends Command {
  ...
  async run(client, ctx, args, color, emoji, language) {
    ...
    if (!giveaway) {
      return ctx.sendMessage({
        content: "No giveaway found with the provided message ID.",
      });
    }

    if (!giveaway.ended) {
      return ctx.sendMessage({
        content: "This giveaway has not ended yet, so it cannot be rerolled.",
      });
    }
    ...
    if (newWinners.length === 0) {
      return ctx.sendMessage({
        content: "No eligible participants left to reroll.",
      });
    }
