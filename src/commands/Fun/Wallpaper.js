    ...
    async run(client, ctx, args) {
        ...
        for (let i = 0; i < totalPages; i++) {
            ...
            const embed = client.embed()
                ...
                .setTitle(`${client.emoji.mainLeft} 𝐖𝐀𝐋𝐋𝐏𝐀𝐏𝐄𝐑𝐒 - ${client.utils.formatCapitalize(selectedCategory)} ${client.emoji.mainRight}`)
        ...
        await paginateWallpapers(client, ctx, pages, selectedCategory, selectedWallpapers);
    }
};

async function paginateWallpapers(client, ctx, pages, selectedCategory, selectedWallpapers) {
    let page = 0;
    ...
    const getButtonRow = () => {
        ...
        const itemSelect = new SelectMenuBuilder()
    ...
    const displayItemDetails = (selectedItemIndex) => {
        ...
        if (!wallpaper) {
            ...
            return { embed: client.embed().setDescription('Wallpaper not found.').setColor('RED') };
        }

        const embed = client.embed()
            .setColor(client.color.main)
            .setTitle(`${client.emoji.mainLeft} 𝐖𝐀𝐋𝐋𝐏𝐀𝐏𝐄𝐑 𝐃𝐄𝐓𝐀𝐈𝐋 : ${wallpaper.name} ${client.emoji.mainRight}`)
            .setThumbnail(ctx.author.displayAvatarURL({ dynamic: true, size: 1024 }))
    ...
    collector.on('collect', async int => {
        if (int.customId === 'prev_item') {
            ...
            if (selectedItemIndex !== -1) {
                ...
                await int.update({ embeds: [client.embed().setDescription('Wallpaper not found.').setColor(client.color.red)], components: getButtonRow().components });
            }
        }
    });
