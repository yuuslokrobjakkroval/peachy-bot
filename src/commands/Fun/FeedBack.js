...
module.exports = class Feedback extends Command {
  ...
  async run(client, ctx, args, color, emoji, language) {
    ...
    const feedbackMessages = language.locales.get(language.defaultLocale)
      ?.informationMessages?.feedbackMessages;
    ...
    if (type === "list") {
      ...
      const userFeedback =
        ctx.options?.getString("feedback") || args.slice(1).join(" ");

      if (!userFeedback) {
        return client.utils.sendErrorMessage(
          client,
          ctx,
          feedbackMessages.noFeedbackProvided,
          color,
          2 * 60 * 60
        );
      }
      ...
      if (!ratingMatch) {
        return client.utils.sendErrorMessage(
          client,
          ctx,
          feedbackMessages.invalidRatingFormat,
          color,
          2 * 60 * 60
        );
      }
      ...
      if (rating < 1 || rating > 10) {
        return client.utils.sendErrorMessage(
          client,
          ctx,
          feedbackMessages.ratingOutOfBounds,
          color,
          2 * 60 * 60
        );
      }
      ...
      const embed = client
        ...
        .setDescription(
          generalMessages.title
            ...
            feedbackMessages.thankYou.replace(
              "%{userName}",
        ...
        .addFields([
          {
            name: feedbackMessages.fields.rating,
            value: `${rating}/10`,
            inline: false,
          },
          {
            name: feedbackMessages.fields.feedback,
            value: feedbackText,
            inline: false,
          },
        ])
