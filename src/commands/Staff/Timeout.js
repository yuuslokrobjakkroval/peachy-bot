...
module.exports = class Timeout extends Command {
    ...
    async run(client, ctx, args, color, emoji, language) {
            ...
            return await ctx.sendMessage({
                embeds: [
                    client
                        ...
                        .setDescription(`User is already in timeout. Timeout ends in ${remainingTime} minute(s).`),
                ],
            });
        }
        ...
        const initialEmbed = await ctx.sendMessage({
            embeds: [client.embed().setColor(color.main).setDescription(`Timed out **${mention}** for \`${duration}\`.`)],
        });

        // Countdown logic
        const countdownInterval = setInterval(async () => {
            const now = new Date();
            const newRemainingTime = Math.ceil((user.verification.timeout.expiresAt - now) / 1000 / 60); // Recalculate remaining time
                ...
                await initialEmbed.edit({
                    embeds: [
                        client
                            .embed()
                            .setColor(color.danger)
                            .setDescription(`User is in timeout. Timeout ends in ${newRemainingTime} minute(s).`),
                    ],
                });
            }
