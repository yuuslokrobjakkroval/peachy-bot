...
module.exports = class Unblacklist extends Command {
    ...
    async run(client, ctx, args, color, emoji, language) {
        ...
        await Users.updateOne({ userId }, { $set: { 'verification.isBlacklist': false } }).exec();

        return await ctx.sendMessage({
            embeds: [client.embed().setColor(color.success).setDescription(`${globalEmoji.result.tick} Un-blacklisted Successfully.`)],
        });
    }
};
