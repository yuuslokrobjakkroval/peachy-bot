    ...
    async run(client, ctx, args, color, emoji, language) {
        ...
        if (args.length < 1) {
            return await ctx.sendMessage({
                embeds: [client.embed().setColor(color.danger).setDescription('Please specify whether to `ban`, and mention a user.')],
            });
        }
        ...
        if (isBanned) {
            ...
            const embed = client
                .embed()
                .setColor(color.main)
                .setDescription(`${globalEmoji.result.tick} Banned **${mention}** for: \`${reason}\``);
