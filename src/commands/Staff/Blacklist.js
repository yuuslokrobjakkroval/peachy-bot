...
module.exports = class Blacklist extends Command {
    ...
    async run(client, ctx, args, color, emoji, language) {
        ...
        if (typeof mention !== 'string' && mention.bot) {
            return await client.utils.sendErrorMessage(client, ctx, 'You cannot blacklist a bot.', color);
        }
        ...
        await Users.updateOne({ userId }, { $set: { 'verification.isBlacklist': true } }, { upsert: true }).exec();

        return await ctx.sendMessage({
            embeds: [client.embed().setColor(color.main).setDescription(`${globalEmoji.result.tick} Blacklisted **Successfully**.`)],
        });
    }
};
