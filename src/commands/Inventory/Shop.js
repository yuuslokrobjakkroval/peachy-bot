...
const globalEmojis = require('../../utils/Emoji.js');
...
module.exports = class Shop extends Command {
    ...
    async run(client, ctx, args, color, emoji, language) {
        ...
        const shopMessages = language.locales.get(language.defaultLocale)?.inventoryMessages?.shopMessages || {};

        // Ensure we have default values for all messages
        const messages = {
            itemDetails: shopMessages.itemDetails || 'Item Details',
            buyCommand: shopMessages.buyCommand || 'Use `/buy [item] [amount]` to purchase',
            ...
            errorLoading: shopMessages.errorLoading || 'An error occurred while loading the shop.',
            categoryNotFound: shopMessages.categoryNotFound || 'Shop category not found.',
            purchaseSuccess: shopMessages.purchaseSuccess || 'You have successfully purchased %{quantity}x %{item} for %{price} coins!',
            purchaseFailed: shopMessages.purchaseFailed || 'Purchase failed: %{reason}',
            insufficientFunds: shopMessages.insufficientFunds || "You don't have enough coins to buy this item.",
            invalidQuantity: shopMessages.invalidQuantity || 'Please enter a valid quantity.',
            confirmPurchase: shopMessages.confirmPurchase || 'Are you sure you want to buy %{quantity}x %{item} for %{price} coins?',
            ...
            if (!selectedShop) {
                ...
                if (!selectedShop) {
                    return await ctx.sendMessage({
                        embeds: [client.embed().setColor(safeColors.error).setDescription(messages.categoryNotFound)],
                    });
                }
            }

            return await this.showShop(client, ctx, safeColors, emoji || {}, uniqueCategories, selectedCategory, generalMessages, messages);
        } catch (error) {
            console.error('Error in Shop command:', error);
            return await ctx.sendMessage({
                embeds: [client.embed().setColor(safeColors.error).setDescription(messages.errorLoading)],
            });
        }
    }

    async showShop(client, ctx, color, emoji, categories, initialCategory, generalMessages, messages) {
        const currentState = {
            ...
            if (currentState.filter && currentState.filter !== 'all') {
                items = items.filter((item) => {
                    if (currentState.filter === 'tool') {
                        ...
                        return item.type === 'color' || item.type === 'decoration' || item.type === 'wallpaper';
        ...
        const getSelectedItem = () => {
            ...
            return getCurrentItems().find((item) => item.id === currentState.selectedItemId);
        };
        ...
        const generateShopListEmbed = () => {
            ...
            const itemList = items
                .map((item, index) => {
                    ...
                    const priceDisplay = item.price.buy > 10000 ? `**${price}** ${emoji.coin || '💰'}` : `${price} ${emoji.coin || '💰'}`;
            ...
            const embed = client
                ...
                .setFooter({
                    text: `Page ${currentState.page + 1}/${totalPages} • Select an item to view details and purchase`,
        ...
        const generateItemDetailsEmbed = () => {
            ...
            const embed = client
                ...
                .setThumbnail(
                    item.emoji ? client.utils.emojiToImage(item.emoji) : ctx.author.displayAvatarURL({ dynamic: true, size: 1024 })
                )
                .addFields([
                    {
                        name: `${messages.id}`,
                    ...
                    {
                        name: `${messages.price}`,
                        value: `${client.utils.formatString(item.price.buy)} ${emoji.coin || '💰'}`,
                        inline: true,
                    },
                    {
                        name: `${messages.type}`,
        ...
        const generatePurchaseConfirmEmbed = () => {
            ...
            const embed = client
                ...
                .addFields([
                    ...
                    {
                        name: 'Total Price',
                        value: `${client.utils.formatString(totalPrice)} ${emoji.coin || '💰'}`,
                        inline: true,
                    },
                ])
        ...
        const generateItemDropdown = () => {
            ...
            const uniqueItems = items.filter((item, index, self) => index === self.findIndex((i) => i.id === item.id));

            const itemOptions = uniqueItems.slice(0, 25).map((item) => {
                const emojiStr = typeof emoji.coin === 'string' ? emoji.coin.replace(/<a?:\w+:(\d+)>/, '') : '💰';
                return {
                    label: item.name,
                    value: item.id,
                    description: `${client.utils.formatString(item.price.buy)} ${emojiStr}`,
                    emoji: item.emoji && item.emoji.includes(':') ? undefined : item.emoji, // Only use Unicode emojis in dropdown
        ...
        const generateNavigationButtons = () => {
            ...
            const homeButton = new ButtonBuilder().setCustomId('home').setLabel('Home').setStyle(ButtonStyle.Secondary).setEmoji('🏠');
            ...
            if (currentState.viewMode === 'details') {
                ...
                if (totalPages > 1) {
                    return new ActionRowBuilder().addComponents(homeButton, prevButton, nextButton);
                } else {
        ...
        const generatePurchaseButtons = () => {
            ...
            const buyButton = new ButtonBuilder()
                ...
                .setEmoji(globalEmojis.shop.buy || '🛒');
        ...
        const generatePurchaseConfirmButtons = () => {
            ...
            const confirmButton = new ButtonBuilder()
                ...
                .setStyle(ButtonStyle.Success)
                .setEmoji(globalEmojis.result.tick || '✅');
            ...
            const cancelButton = new ButtonBuilder()
                ...
                .setEmoji(globalEmojis.result.deny || '❌');
            ...
            const increaseButton = new ButtonBuilder().setCustomId('increase_quantity').setLabel('+').setStyle(ButtonStyle.Secondary);
            ...
            const row1 = new ActionRowBuilder().addComponents(decreaseButton, quantityButton, increaseButton);
        ...
        const updateMessage = async (interaction = null, skipInteractionUpdate = false) => {
            try {
                const components = [];
                ...
                if (currentState.needsUpdate) {
                    try {
                        const errorEmbed = client
                            ...
                            .setTitle('Error')
                            .setDescription('An error occurred while updating the shop display. Please try again.');
        ...
        const handlePurchase = async (interaction) => {
            try {
                ...
                if (user.balance.coin < totalPrice) {
                    await interaction.reply({
                        content: messages.insufficientFunds || "You don't have enough coins to buy this item.",
                        flags: 64,
                    });
                    return false;
                }
                ...
                const existingItem = user.inventory.find((invItem) => invItem.id === item.id);
                ...
                const successMessage = (
                    messages.purchaseSuccess || 'You have successfully purchased %{quantity}x %{item} for %{price} coins!'
                )
                    .replace('%{quantity}', quantity.toString())
                    .replace('%{item}', item.name)
                    .replace('%{price}', client.utils.formatString(totalPrice));
                ...
                const errorMessage = (messages.purchaseFailed || 'Purchase failed: %{reason}').replace(
                    '%{reason}',
        ...
        collector.on('collect', async (interaction) => {
            try {
                ...
                if (!currentState.needsUpdate) {
                    ...
                    setTimeout(async () => {
                        try {
                            const errorEmbed = client.embed().setColor(color.error).setDescription('An error occurred during interaction.');
