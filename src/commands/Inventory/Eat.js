...
module.exports = class Eat extends Command {
  ...
  async run(client, ctx, args, color, emoji, language) {
    ...
    const eatMessages = language.locales.get(language.defaultLocale)
      ?.inventoryMessages?.eatMessages;
    ...
    try {
      ...
      if (!user || user.inventory.length === 0) {
        return await client.utils.sendErrorMessage(
          client,
          ctx,
          eatMessages.emptyInventory,
          color,
        );
      }

      const itemId = ctx.isInteraction
        ? ctx.interaction.options.data[0]?.value.toString()
        : args[0];
      ...
      if (!itemInfo) {
        return await client.utils.sendErrorMessage(
          client,
          ctx,
          eatMessages.itemNotFound.replace("%{itemId}", itemId),
          color,
        );
      }
      ...
      const consumedItem = user.consumedItems.find(
        (item) => item.id === itemId,
      );
      if (consumedItem) {
        consumedItem.quantity += itemAmount;
      } else {
        user.consumedItems.push({
          id: itemId,
          name: itemInfo.name,
          type: itemInfo.type,
          quantity: itemAmount,
