        ...
        try {
            client.utils.getUser(ctx.author.id).then(user => {
                if (!user || !user.inventory) {
                    return client.utils.sendErrorMessage(client, ctx, invMessages.noInventory || 'No inventory data found for this user.', color);
                }
                ...
                user.inventory.forEach(item => {
                    if (item.quantity > 0) {
                        ...
                        if (itemInfo) {
                            ...
                            itemList[type].push(
                                `\`${itemInfo.id}\` ${itemInfo.emoji} **${item.quantity}** ${itemInfo.name ? itemInfo.name : client.utils.toNameCase(itemInfo.id)}`
                            );
            ...
            await client.utils.sendErrorMessage(client, ctx, invMessages.error || 'An error occurred while retrieving your inventory.', color);
        }
    }
};
