...
module.exports = class UserInfo extends Command {
    ...
    async run(client, ctx, args, color, emoji, language) {
        ...
        const bannerURL = fetchedUser?.bannerURL({ format: 'png', size: 1024 }) || client.config.links.banner;

        // Create the main embed
        const mainEmbed = this.createMainEmbed(client, ctx, user, guildMember, guild, emoji, color, generalMessages, globalEmoji);

        // Create buttons
        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId('avatar').setLabel('Avatar').setStyle(ButtonStyle.Secondary).setEmoji('🖼️'),
    ...
    createMainEmbed(client, ctx, user, guildMember, guild, emoji, color, generalMessages, globalEmoji) {
        return client
            .embed()
            .setColor(color.main)
            ...
            .setImage(user.bannerURL({ format: 'png', size: 1024 }) || client.config.links.banner)
            .addFields([
                {
                    name: `🆔 ID`,
                ...
                },
                {
                    ...
                    value: `**${globalEmoji.arrow} ${
                        guildMember?.joinedTimestamp
                            ? `${Math.floor((Date.now() - guildMember.joinedTimestamp) / (1000 * 60 * 60 * 24))} days ago`
                            : 'N/A'
                    }**`,
                    inline: false,
                },
                ...
                },
                {
                    ...
                    value: `${globalEmoji.arrow} **${guildMember?.premiumSince ? 'True' : 'False'}**`,
                    inline: false,
                },
                {
                    name: `⭐ Top role`,
                    value: `${globalEmoji.arrow} **${guildMember?.roles.highest.name || 'None'}**`,
                    inline: false,
                },
                {
                    name: '📅 Created At',
