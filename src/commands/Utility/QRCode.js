...
module.exports = class QRCode extends Command {
	...
	async run(client, ctx, args, color, emoji, language) {
		...
		const qrMessages = language.locales.get(language.defaultLocale)
			?.utilityMessages?.qrMessages; // Access qrMessages
		...
		if (currency === "kh") {
			...
			return ctx.isInteraction
				? await ctx.interaction.editReply({
						content:
							qrMessages?.invalidOption ||
							"Invalid option! Please use 'kh' or 'usd'.",
						flags: 64,
					})
		...
		const embed = client
			...
			.setColor(color.main)
			.setTitle(
				qrMessages?.title.replace("%{currency}", currency.toUpperCase()) ||
					`QR Code for ${currency.toUpperCase()}`,
			)
			.setDescription(
				qrMessages?.description.replace(
					"%{currency}",
