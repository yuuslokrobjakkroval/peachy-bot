  ...
  async run(client, ctx, args, color, emoji, language) {
    ...
    if (user.verification.verify.status === "verified") {
      return await client.utils.sendErrorMessage(
        client,
        ctx,
        verifyMessages?.alreadyVerified || "You are already verified.",
        color,
      );
    }
    ...
    const confirmButton = client.utils.labelButton(
      "confirm",
      verifyMessages?.confirmButtonLabel || "Confirm",
      3,
    );
    const cancelButton = client.utils.labelButton(
      "cancel",
      verifyMessages?.cancelButtonLabel || "Cancel",
      4,
    );
    ...
    const confirmEmbed = client
      ...
      .setColor(color.main)
      .setTitle(
        `${emoji.mainLeft} ${verifyMessages?.title || "Verify Account"} ${emoji.mainRight}`,
      )
      .setDescription(
      ...
      .setFooter({
        text: `${verifyMessages?.requestedBy || "Requested By"} ${
    ...
    const filter = (i) =>
      i.user.id ===
      (ctx.isInteraction ? ctx.interaction.user.id : ctx.author.id);
    ...
    collector.on("collect", async (i) => {
      ...
      if (i.customId === "confirm") {
        ...
        const verificationEmbed = client
          .embed()
          .setColor(color.main)
          .setTitle(
            `${emoji.mainLeft} ${verifyMessages?.verificationRequest || "Verification Request"} ${emoji.mainRight}`,
          )
          .setDescription(
            `${
              verifyMessages?.verificationMessage ||
              "You have requested to verify with PEACHY."
            }\n` +
              (userPaymentStatus === "paid"
                ? `${
                    verifyMessages?.submitButtonMessage ||
                    "Click the button below to submit your verification code."
                  }`
                : `${
                    verifyMessages?.waitForPayment ||
                    "If you've made a payment, please wait a moment. If not, please send a message to <@966688007493140591>."
                  }`),
          )
          .setFooter({
        ...
        const submitButton = client.utils.labelButton(
          `submit_${codeNumber}`,
          verifyMessages?.submitButton || "Submit",
          3,
        );

        const verificationRow =
          userPaymentStatus === "paid" &&
          client.utils.createButtonRow(submitButton);
        await i.update({
    ...
    const paymentFilter = (i) =>
      i.customId.startsWith("submit_") &&
      i.user.id ===
        (ctx.isInteraction ? ctx.interaction.user.id : ctx.author.id);
    const paymentCollector = ctx.channel.createMessageComponentCollector({
    ...
    paymentCollector.on("collect", async (i) => {
      ...
      const inputField = new TextInputBuilder()
        ...
        .setLabel(
          `${
            verifyMessages?.verificationCodeLabel || "Verification Code"
          }: ${submittedCode}`,
        )
        .setStyle(TextInputStyle.Short)
        .setPlaceholder(
          verifyMessages?.placeholder || "Enter your verification code here",
        )
        .setRequired(true);
      ...
      client.once("interactionCreate", async (modalInteraction) => {
        ...
        if (modalInteraction.customId === "inputCodeModal") {
          const inputCode = modalInteraction.fields.getTextInputValue(
          ...
          if (inputCode === verificationCode) {
              ...
              { userId: ctx.author.id },
              {
                $set: {
                  ...
                  "verification.verify.message":
                    verifyMessages?.successMessage ||
                    "Thank you for supporting and verifying with PEACHY!",
                },
              },
            );
            ...
            const successEmbed = client
              ...
              .setTitle(
                `${emoji.mainLeft} ${
                  verifyMessages?.verificationSuccessful ||
                  "Verification Successful"
                } ${emoji.mainRight}`,
              )
              .setDescription(
                `${
                  verifyMessages?.thankYouForVerifying ||
                  "Thank you for supporting and verifying!"
                } ${globalEmoji.verify}\n `,
              )
            ...
            const messageToUpdate =
              await modalInteraction.channel.messages.fetch(i.message.id);
            await messageToUpdate.edit({
              content:
                verifyMessages?.verificationComplete ||
