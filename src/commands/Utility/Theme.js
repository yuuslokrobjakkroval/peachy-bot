...
module.exports = class Theme extends Command {
  constructor(client) {
    super(client, {
      ...
      description: {
        content:
          "Toggle between peach, goma, and normal themes or view your current theme setting.",
        examples: [
  ...
  async run(client, ctx, args, color, emoji, language) {
    ...
    const themeMessages = language.locales.get(language.defaultLocale)
      ?.utilityMessages?.themeMessages;
    ...
    try {
      ...
      switch (convertSubCommand) {
        ...
        case "peach":
        case "goma": {
          const equippedItem = user.equip.find(
            (item) => item.id.startsWith("t") || item.id.startsWith("st"),
          );
          if (equippedItem) {
          ...
          const themeColor =
            convertSubCommand === "peach"
              ? "#8BD3DD"
              : convertSubCommand === "goma"
                ? "#94716B"
                : "#F582AE";
          const themeImage =
            convertSubCommand === "peach"
              ? globalGif.welcomeToPeach
              : convertSubCommand === "goma"
                ? globalGif.welcomeToGoma
                : globalGif.welcomeToPeachAndGoma;
          embed
            .setColor(themeColor)
            .setDescription(
              generalMessages.title
                ...
                themeMessages?.setThemeMessage.replace(
                  "%{theme}",
                  client.utils.formatCapitalize(convertSubCommand),
                ),
            )
            .setImage(themeImage)
            .setFooter({
        ...
        case "help": {
          ...
          embed
            ...
            .addFields([
              {
                name: themeMessages?.examplesLabel || "Examples",
                value: examples.join("\n"),
              },
          ...
          embed
            ...
            .setDescription(
              generalMessages.title
                ...
                .replace("%{title}", "THEME SETTING")
                .replace("%{mainRight}", emoji.mainRight) +
                themeMessages?.currentThemeMessage.replace(
                  "%{theme}",
                  client.utils.formatCapitalize(currentTheme),
                ),
            )
            .setImage(themeImage)
            .setFooter({
