...
module.exports = class ServerInfo extends Command {
    ...
    async run(client, ctx, args, color, emoji, language) {
        ...
        const approximateMemberCount = guild.approximateMemberCount || guild.memberCount;

        // Get presence counts from cached members only (to avoid fetching all members)
        const cachedMembers = guild.members.cache;
        const onlineCount = cachedMembers.filter((member) => member.presence?.status === 'online').size;
        const idleCount = cachedMembers.filter((member) => member.presence?.status === 'idle').size;
        const dndCount = cachedMembers.filter((member) => member.presence?.status === 'dnd').size;
        const offlineCount = cachedMembers.filter((member) => !member.presence || member.presence?.status === 'offline').size;
        ...
        const offline7Days = cachedMembers.filter((member) => !member.presence && member.joinedTimestamp < sevenDaysAgo).size;

        const offline30Days = cachedMembers.filter((member) => !member.presence && member.joinedTimestamp < thirtyDaysAgo).size;
        ...
        const bannerURL = guild.bannerURL({ size: 1024 }) || client.config.links.banner;
        ...
        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId('icon').setLabel('Avatar').setStyle(ButtonStyle.Secondary).setEmoji('🖼️'),
            new ButtonBuilder().setCustomId('banner').setLabel('Banner').setStyle(ButtonStyle.Secondary).setEmoji('🏞️')
        );
        ...
        collector.on('collect', async (interaction) => {
            ...
            if (interaction.customId === 'icon') {
                ...
                const iconRow = new ActionRowBuilder().addComponents(
                    new ButtonBuilder().setCustomId('home').setLabel('Home').setStyle(ButtonStyle.Secondary).setEmoji('🏠'),
                    new ButtonBuilder().setCustomId('banner').setLabel('Banner').setStyle(ButtonStyle.Secondary).setEmoji('🏞️')
                );
    ...
    createMainEmbed(
        ...
        return client
            ...
            .addFields([
                ...
                {
                    name: '🔒 Verification',
                    value: `${globalEmoji.arrow} ${verificationLevels[guild.verificationLevel]}`,
                    inline: false,
                },
                {
                    name: '📊 Channel',
                    ...
                    }**\nVoice channels ${globalEmoji.arrow} **${guild.channels.cache.filter((ch) => ch.type === 2).size}**`,
                    inline: false,
                },
                {
