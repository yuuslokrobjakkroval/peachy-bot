...
module.exports = class AdminHelp extends Command {
    ...
    async run(client, ctx, args, color, emoji, language) {
        ...
        const commands = client.commands.filter((cmd) => adminCategory.some((cat) => cat.name === cmd.category.toLowerCase()));
        ...
        if (!args[0]) {
            ...
            collector.on('collect', async (interaction) => {
                ...
                const categoryCommands = commands.filter((cmd) => selectedCategories.includes(cmd.category.toLowerCase()));
                ...
                selectedCategories.forEach((selectedCategory) => {
                    const catCommands = commands.filter((cmd) => cmd.category.toLowerCase() === selectedCategory);

                    if (catCommands.size > 0) {
                        const categoryInfo = adminCategory.find((cat) => cat.name === selectedCategory);
                ...
                const categorySelectMenu = new StringSelectMenuBuilder()
                    .setCustomId('category_select')
                    .setPlaceholder(`🎯 ${selectedCategories.length} selected • Choose more... ✨`)
                    .setMinValues(1)
                    .setMaxValues(adminCategory.length)
                    .addOptions(categoryOptions);
            ...
            // Get category emoji
            const categoryInfo = adminCategory.find((cat) => cat.name === command.category.toLowerCase());
            const categoryEmoji = categoryInfo ? categoryInfo.emoji : '📁';

            const helpEmbed = client
                .embed()
                .setColor(color.main)
                .setTitle(`${categoryEmoji} ${client.utils.formatCapitalize(command.name)} Command`)
                ...
                .addFields([
                    ...
                    {
                        name: `🏷️ Aliases`,
                        value: command.aliases.length > 0 ? command.aliases.map((alias) => `\`${alias}\``).join(', ') : '`None`',
                        inline: true,
                    },
                    {
                        name: `⏱️ Cooldown`,
                    ...
                    },
                    {
                        ...
                        value:
                            command.permissions.client.length > 0
                                ? command.permissions.client.map((perm) => `\`${perm}\``).join(', ')
                                : '`None required`',
                        inline: false,
                    },
                    {
