    ...
    async run(client, ctx, args, color, emoji, language) {
        try {
            ...
            if (usersWithLevelSettings.length === 0) {
                return await client.utils.oops(client, ctx, 'Level system is not set up. There is nothing to disable.', color);
            }
            ...
            await client.utils.oops(client, ctx, 'An error occurred while disabling the level system.', color);
        }
    }
};
