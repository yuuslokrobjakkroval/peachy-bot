    ...
    async run(client, ctx, args, language) {
        ...
        switch (subCommand) {
            case 'set': {
                ...
                if (!channel) {
                    return await client.utils.oops(client, ctx, 'Please specify a valid channel.');
                }
            ...
            case 'view': {
                ...
                if (users.length === 0) {
                    return await client.utils.oops(client, ctx, 'No level system is set up.');
                }
            ...
            default:
                await client.utils.oops(client, ctx, `Please provide a valid subcommand for the \`${this.name}\` command.`);
                break;
        }
    }
};
