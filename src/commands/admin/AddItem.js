...
const allItems = Items.concat(ImportantItems, Woods, Minerals, Fishs, Slimes, Bugs, Crops, Tools);
...
module.exports = class AddItem extends Command {
    ...
    async run(client, ctx, args, color, emoji, language) {
        ...
        if (itemIndex !== -1) {
            await Users.updateOne({ userId: userId, 'inventory.id': itemId }, { $inc: { 'inventory.$.quantity': baseQuantity } }).exec();
        } else {
        ...
        const embed = client
            .embed()
            .setColor(color.main)
            .setDescription(`${globalEmoji.result.tick} Added ${itemInfo.emoji} **x${baseQuantity}** ${itemInfo.id} to ${mention}.`);
