...
module.exports = class Gift extends Command {
  ...
  async run(client, ctx, args, color, emoji, language) {
    ...
    const boxType = ctx.isInteraction
      ? ctx.interaction.options.getString("box_type")
      : args[0];
    const amount = ctx.isInteraction
      ? ctx.interaction.options.getInteger("amount")
      : args[1];
    const specifiedChannel = ctx.isInteraction
      ? ctx.interaction.options.getChannel("channel")
      : args[2];

    if (
      !boxType ||
      !["common", "rare", "epic", "legendary", "mythic"].includes(boxType) ||
      isNaN(amount) ||
      amount <= 0
    ) {
    ...
    if (specifiedChannel) {
      ...
      channels = ctx.guild.channels.cache.filter((channel) =>
    ...
    for (const selectedChannel of channels) {
      ...
      const giftMessage = await selectedChannel
        .send({
          embeds: [giftEmbed],
          components: [row],
        })
        .catch((err) => {
          console.error(
            `Could not send message to ${selectedChannel.name}:`,
            err
          );
        });
      ...
      collector.on("collect", async (interaction) => {
        ...
        if (interaction.customId === "claim_gift") {
          ...
          const rewardSummary = await addRewardToUserInventory(
            client,
            interaction,
            color,
            emoji,
          ...
          const successEmbed = this.client
            .embed()
            .setColor(color.main)
            .setThumbnail(gif.giftClaimed)
            .setTitle(
              `****${
                interaction.displayName || interaction.user.displayName
              }**** Claimed!`
            )
          ...
          const claimedUsers = await Promise.all(
            claimedGifts[selectedChannel.id].map((userId) => {
              return client.users
                .fetch(userId)
                .then((user) => {
          ...
          const totalClaims = Object.values(userClaimCounts).reduce(
            (total, count) => total + count,
            0
          ); // Total claims made by all users
          // Total number of boxes
          const responseMessage =
            `Gift boxes have been sent to ****${amount}**** random channels: ${sentChannelNames.join(
              ", "
            )}!\n` +
...
async function addRewardToUserInventory(
  client,
  interaction,
  color,
  emoji,
  ...
  try {
    ...
    if (reward.amount) {
      ...
      rewardSummary.push(
        `****${client.utils.formatNumber(reward.amount)}**** ${emoji.coin}`
      );
    }
