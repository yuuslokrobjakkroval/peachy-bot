...
module.exports = class ApplyJobList extends Command {
    constructor(client) {
        super(client, {
            name: 'applyjoblist',
            description: {
                content: 'List all users with pending job applications grouped by position.',
    ...
    async run(client, ctx, args, color, emoji, language) {
        try {
            ...
            if (users.length === 0) {
                return client.utils.sendErrorMessage(client, ctx, 'No users with pending job applications.', color);
            }
            ...
            const groupedByPosition = users.reduce((acc, user) => {
                ...
                acc[position].push(
                    `**ID:** ${user.userId}\n**Name:** ${user.username || 'Unknown'}\n**Applied Date:** ${new Date(user.work.applyDate).toLocaleDateString()}\n`
                );
                return acc;
            }, {});
            ...
            for (const [position, userList] of Object.entries(groupedByPosition)) {
                ...
                chunks.forEach((chunk, index) => {
                    const embed = client.embed()
                        .setColor(color.main)
                        .setTitle(`Apply For: ${position === 'it' ? 'IT' : client.utils.formatCapitalize(position)}`)
