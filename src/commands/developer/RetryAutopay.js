...
module.exports = class ReAutopay extends Command {
  constructor(client) {
    super(client, {
      ...
      description: {
        content:
          "Re-attempt autopay for a specific giveaway using the message ID.",
  ...
  async run(client, ctx, args, color, emoji, language) {
    ...
    try {
        ...
        return await this.sendReply(ctx, {
          embeds: [
            client
              ...
              .setColor(color.danger)
              .setDescription(
                `No giveaway found with message ID: ${messageId}`
              ),
          ],
        });
      ...
      const winners =
        giveaway.winnerId.length === 0 ? giveaway.entered : giveaway.winnerId;
      ...
      for (const winnerId of winners) {
        try {
          ...
          await ctx.channel.send(
            `⚠️ Failed to autopay for user <@${winnerId}>.`
          );
        }
      }
      ...
      await this.sendReply(ctx, {
        embeds: [
          client
            ...
            .setColor(color.danger)
            .setDescription(
              "An error occurred while reprocessing the autopay."
            ),
        ],
      });
    }
