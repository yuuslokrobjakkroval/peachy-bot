    ...
    run(client, ctx, args, color, emoji, language) {
        try {
            ...
            client.guilds.cache.forEach(guild => {
                ...
                if (channel) {
                    result.push(`**Server Name:** ${guild.name} (ID: ${guild.id})\n**Channel Name:** <#${channelId}> (${channel.name})\n**Channel ID:** ${channelId}`);
                }
            });

            if (result.length === 0) {
                return ctx.sendMessage({
                    content: 'The specified channel ID was not found in any of the servers the bot is in.',
                    ephemeral: true,
                });
            }
            ...
            return client.utils.sendErrorMessage(client, ctx, 'An error occurred while checking the channel in all servers.', color);
        }
    }
};
