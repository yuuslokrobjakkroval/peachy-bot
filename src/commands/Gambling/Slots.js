...
module.exports = class Slots extends Command {
	...
	run(client, ctx, args, color, emoji, language) {
		...
		Users.findOne({ userId: ctx.author.id }).exec().then((user) => {
			...
			Users.updateOne({ userId: ctx.author.id }, { $set: { 'balance.coin': coin - baseCoins, 'balance.bank': bank } }).exec();

			// ===================================== > Decide Results < ===================================== \\
			let rslots = [];
			random(1, 1000).then((rand) => {
				rand /= 10;
				let win = 0;
				...
				ctx.sendMessage({ embeds: [initialEmbed] }).then(() => {
					...
					const spinSecondEmbed = client.embed()
						.setColor(color.main)
						.setThumbnail(ctx.author.displayAvatarURL({ dynamic: true, size: 1024 }))
						.setDescription(`# **${emoji.mainLeft} 𝐒𝐋𝐎𝐓𝐒 ${emoji.mainRight}**\n ### \`╭┈ • ┈ ୨୧ ┈ • ┈╮\`\n ## **\`|\`     ${rslots[0]} ${emoji.slots.spin} ${rslots[1]}     \`|\`**\n ### \`╰┈ • ┈ ୨୧ ┈ • ┈╯\`\n**\nYou bet \`${numeral(baseCoins).format()}\` ${emoji.coin}**\n`)
						.setFooter({
							text: `${ctx.author.displayName}, your game is in progress!`,
							iconURL: verify ? client.utils.emojiToImage(emojiImage.verify) : ctx.author.displayAvatarURL(),
						});
					...
					const resultEmbed = client.embed()
						.setColor(win ? color.success : color.error)
						.setThumbnail(ctx.author.displayAvatarURL({ dynamic: true, size: 1024 }))
