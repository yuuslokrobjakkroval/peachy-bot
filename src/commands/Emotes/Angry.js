...
module.exports = class Angry extends Command {
	...
	async run(client, ctx, args, color, emoji, language) {
		...
		const angryMessages = language.locales.get(language.defaultLocale)
			?.emoteMessages?.angryMessages;
		const errorMessages = angryMessages.errors;

		try {
			const randomEmoji = client.utils.getRandomElement(
				emoji.emotes && emoji.emotes.angry
					? emoji.emotes.angry
					: globalEmoji.emotes.angry,
			);
			...
			const embed = client
				...
				.setDescription(
					generalMessages.title
						...
						angryMessages.description.replace(
							"%{user}",
