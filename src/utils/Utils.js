...
module.exports = class Utils {
    ...
    static async getCachedUser(userId, ttlMs = 60000) {
        ...
        if (now - Utils.cache.lastCacheClean > 300000) {
            Utils.cleanCache();
            Utils.cache.lastCacheClean = now;
        }

        const cachedUser = Utils.cache.users.get(userId);
        ...
        const user = await Users.findOne({ userId }).exec();
        if (user) {
            Utils.cache.users.set(userId, { data: user, timestamp: now });
        }
        return user;
    }
    ...
    static cleanCache() {
        const now = Date.now();
        for (const [userId, entry] of Utils.cache.users.entries()) {
            if (now - entry.timestamp > 300000) {
                // 5 minutes TTL
                Utils.cache.users.delete(userId);
            }
        }
    }
    ...
    static cacheItems() {
        ...
        return Utils.cache.items;
    }

    // Standardized error handling
    static errorMessages = {
        INSUFFICIENT_FUNDS: "You don't have enough coins for this action. Try earning more with daily rewards or by selling items.",
        ITEM_NOT_FOUND: 'The requested item was not found. Check the item ID and try again.',
        COOLDOWN_ACTIVE: 'This command is on cooldown. Please wait before trying again.',
        PERMISSION_DENIED: "You don't have permission to use this command.",
        INVALID_ARGUMENTS: 'Invalid command arguments. Please check the command usage.',
    ...
    static getErrorMessage(errorCode, ...args) {
        const message = Utils.errorMessages[errorCode] || Utils.errorMessages.GENERIC_ERROR;
    ...
    static validateInput(input, type, options = {}) {
        switch (type) {
            ...
            case 'string':
                ...
                if (options.minLength !== undefined && input.length < options.minLength) return false;
                if (options.maxLength !== undefined && input.length > options.maxLength) return false;
            ...
            case 'itemId':
                ...
                if (!Utils.cache.items) Utils.cacheItems();
                return Utils.cache.items.some((item) => item.id === input);
    ...
    static checkRateLimit(userId, commandName, limit = 5, window = 60000) {
        ...
        if (!Utils.rateLimits.has(key)) {
            Utils.rateLimits.set(key, []);
        }

        const userHistory = Utils.rateLimits.get(key);

        // Remove timestamps outside the current window
        const validHistory = userHistory.filter((timestamp) => now - timestamp < window);
        Utils.rateLimits.set(key, validHistory);
    ...
    static async checkAndSetCooldown(client, ctx, command, cooldownTime) {
        ...
        const user = await Utils.getCachedUser(userId);
        if (!user) return false;

        // Check if user is on cooldown
        const cooldown = user.cooldowns.find((c) => c.name === command.name.toLowerCase());
        const isOnCooldown = cooldown ? Date.now() - cooldown.timestamp < cooldownTime : false;

        if (!isOnCooldown) {
            // Set cooldown
            await Utils.updateUserWithRetry(userId, async (user) => {
                const existingCooldown = user.cooldowns.find((c) => c.name === command.name.toLowerCase());
                if (existingCooldown) {
            ...
            await Utils.sendErrorMessage(
                client,
                ctx,
                `Please wait <t:${Math.round(Date.now() / 1000) + remainingTime}:R> before using this command again.`,
                client.color.danger,
                remainingTime * 1000
            );
    ...
    static calculateNextLevelXpBonus(level) {
        ...
        return Math.floor(base * scalingFactor ** (level - 1));
    }
    ...
    static getCheckingUser(client, message, user, color, emoji, prefix) {
        if (user) {
            ...
            if (!user.profile.lastXpGain || now - user.profile.lastXpGain >= xpCooldown) {
                const creditGained = message.content.startsWith(prefix) || message.content.startsWith(prefix.toLowerCase()) ? 1 : 0;

                user.balance.credit += creditGained;

                const xpGained =
                    message.content.startsWith(prefix) || message.content.startsWith(prefix.toLowerCase())
    ...
    static async getVoiceCheckingUser(client, member, user, color, emoji) {
        ...
        const xpGained = isMainGuild ? client.utils.getRandomNumber(8, 10) : client.utils.getRandomNumber(7, 8);

        user.profile.voiceXP += xpGained;
        user.profile.lastVoiceXpGain = now;
        user.profile.lastVoiceActivity = new Date();

        const nextLevelXp = client.utils.calculateNextLevelXpBonus(user.profile.voiceLevel);

        if (user.profile.voiceXP >= nextLevelXp) {
            user.profile.voiceXP -= nextLevelXp;
            user.profile.voiceLevel += 1;
            user.profile.voiceLevelXp = client.utils.calculateNextLevelXpBonus(user.profile.voiceLevel);
    ...
    static cooldown(id, timeout, cdId, cooldowntime, message, cooldowns, prem) {
        if (id === Utils.client.config.ownerId) {
            return false;
        }
                    ...
                    message.channel
                        .send({
                            embeds: [
                                new EmbedBuilder()
                                    .setColor('Blue')
                                    .setDescription(`<@${id}> cooldown **<t:${Math.floor(cooldownEnd.getTime() / 1000)}:R>**`),
                            ],
                        })
    ...
    static formatBalance(client, ctx, color, coin, amount, invalidAmount) {
        if (!amount || typeof amount !== 'string') {
            return ctx.sendMessage({
                embeds: [client.embed().setColor(color.danger).setDescription(invalidAmount)],
            });
        }
    ...
    static formatString(number, decimals = 2) {
        ...
        const formattedNumber = Number.parseFloat((number / k ** i).toFixed(dm));
        return dm === 0 ? formattedNumber.toFixed(0) : formattedNumber + sizes[i];
    }

    static linkButton(label, link) {
        return new ButtonBuilder().setLabel(`${label}`).setStyle(5).setURL(`${link}`);
    }
    ...
    static fullOptionButton(id, emoji, label, style, disabled = false) {
        const button = new ButtonBuilder().setCustomId(id).setStyle(style).setDisabled(disabled);
    ...
    static async checkBirthdays(client) {
        ...
        try {
            ...
            const todayDate = `${today.getDate().toString().padStart(2, '0')} ${today.toLocaleString('en-US', {
                month: 'short',
            })}`;
            ...
            for (const user of usersWithBirthdayToday) {
                try {
                    ...
                    const xp =
                        Number.parseInt(day) + (new Date(Date.parse(`${month} 1, 2020`)).getMonth() + 1) + Number.parseInt(year.slice(-2));
                ...
                client
                    ...
                    .setDescription(
                        winnerIdArray.length
                            ? `Congratulations ${globalEmoji.congratulation}\n${winnerIdArray
                                  .map((user) => `<@${user}>`)
                                  .join(', ')} ! You have won **${client.utils.formatNumber(data.prize)}** ${emoji.coin} ${
                                  autopay
                                      ? ``
            ...
            embeds: [
                client
                    ...
                    .setDescription(
                        winnerIdArray.length
                            ? `Congratulations ${globalEmoji.congratulation}` +
                                  `${winnerIdArray.map((user) => `<@${user}>`).join(', ')} ! You have won **${
                                      itemInfo.name
                            ...
                            : `No one entered the giveaway for ${itemInfo.name} **\`${client.utils.formatNumber(data.amount)}\`** ${
                                  itemInfo.emoji
                              }!`
                    )
                    .setFooter({
        ...
        try {
            ...
            for (const giveaway of scheduledGiveaways) {
                try {
                    ...
                    if (!channel) {
                        console.error(`Channel ${giveaway.channelId} not found for scheduled giveaway ${giveaway._id}`);
                        continue;
                    }
                    ...
                    const giveawayEmbed = client
                        .embed()
                        .setColor(color.main)
                        .setTitle(`**${client.utils.formatNumber(giveaway.prize)}** ${emoji.coin}`)
                        .setDescription(
                    ...
                    if (giveaway.thumbnail) giveawayEmbed.setThumbnail(giveaway.thumbnail);
                    ...
                    console.log(`Started scheduled giveaway ${giveaway._id} in channel ${channel.name}`);
                } catch (error) {
                    console.error(`Error starting scheduled giveaway ${giveaway._id}:`, error);
                }
            }
            ...
            for (const giveaway of scheduledItemGiveaways) {
                try {
                    ...
                    console.error(`Error starting scheduled item giveaway ${giveaway._id}:`, error);
                }
            }
        } catch (error) {
    ...
    static async checkGiveawayRequirements(client, user, giveaway, interaction) {
        ...
        if (giveaway.requirements.minLevel > 0) {
            ...
            if (!userData || userData.profile.level < giveaway.requirements.minLevel) {
        ...
        if (giveaway.requirements.requiredRole) {
            ...
            if (!member || !member.roles.cache.has(giveaway.requirements.requiredRole)) {
                await interaction.reply({
    ...
    static async validateCommonParams(ctx, client, color, durationStr, winners) {
        ...
        if (isNaN(winners) || winners <= 0) {
            await Utils.sendErrorMessage(client, ctx, 'Number of winners must be bigger than 0', color);
            return { success: false };
        }
    ...
    static async checkBooster(client) {
        ...
        try {
            ...
            const guild = await client.guilds.fetch(guildId).catch((err) => {
                console.error(`[Booster] Error fetching guild ${guildId}: ${err.message}`);
                return null;
            });
            ...
            const rewardChannel = await client.channels.fetch(rewardChannelId).catch((err) => {
                console.error(`[Booster] Error fetching reward channel ${rewardChannelId}: ${err.message}`);
                return null;
            });

            if (!rewardChannel) {
                console.error(`[Booster] Reward channel ${rewardChannelId} not found or inaccessible.`);
                return;
            }

            const boosterMembers = members.filter((member) => roleIds.some((roleId) => member.roles.cache.has(roleId)));
            ...
            for (const member of boosterMembers.values()) {
                try {
                    ...
                    if (user.profile.boosterAcknowledged) {
                        console.log(`[Booster] User ${user.profile.username || user.username} already rewarded.`);
                        continue;
                    }
                    const boosterEmbed = client
                        ...
                        .setTitle(`🎉 Thank You for Boosting/Sponsoring, ${user.profile.username || user.username}!`)
                        .setDescription(`We appreciate your support for keeping our server thriving!`)
                        .addFields([
                            {
                                name: 'Your Reward 🎁',
                    ...
                    console.error(`[Booster] Error processing user ${member.id}: ${userError.message}`);
                }
            }
    ...
    static async generateAIResponse(messageContent, conversationHistory = []) {
        ...
        try {
            ...
            const messages = [systemPrompt, ...conversationHistory, { role: 'user', content: messageContent }];
            ...
            return response.data.candidates[0].content.parts[0].text || 'សូមអភ័យទោស មិនអាចបង្កើតការឆ្លើយតបបានទេ។';
        } catch (error) {
    ...
    static async createGiveaway(client) {
        ...
        try {
            ...
            console.log(`📦 Found ${scheduledGiveaways.length} active scheduled giveaways`);
                ...
                for (const schedule of giveaway.schedules) {
                    ...
                    try {
                        ...
                        if (!channel || channel.guild.id !== giveaway.guildId) {
                            throw new Error("Channel not found or doesn't belong to this guild.");
                        }
                    ...
                    const embed = client
                        .embed()
                        .setColor(client.color.main)
                        .setTitle(schedule.content || `**${client.utils.formatNumber(schedule.prize)}** ${client.emoji.coin}`)
                        .setDescription(
                        ...
                        .setImage(schedule?.image ? schedule.image : 'https://i.imgur.com/khmmAUe.gifv');

                    const joinButton = client.utils.fullOptionButton('giveaway-join', client.emoji.main, '0', 1, false);
                    const participantsButton = client.utils.fullOptionButton('giveaway-participants', '', 'Participants', 2, false);
                    const buttonRow = client.utils.createButtonRow(joinButton, participantsButton);
                    ...
                    try {
                        ...
                        setTimeout(async () => {
                            try {
                                ...
                                console.log(`✅ Sent @everyone ping to channel: ${channel.name}`);
                            } catch (err) {
                                console.error(`❌ Failed to send @everyone ping in channel ${channel.id}`, err);
                            }
                        }, 2000);
                    } catch (sendErr) {
                        console.error(`❌ Failed to send message in channel ${channel.id}`, sendErr);
                        continue;
                    }
                    ...
                    try {
                        ...
                        console.log(`💾 Giveaway saved to DB: ${JSON.stringify(newGiveaway)}`);
                    } catch (dbErr) {
     ...
     */
    static async safeLoadImage(primaryUrl, fallbackUrl = null, fallbackColor = '#DFF2EB') {
