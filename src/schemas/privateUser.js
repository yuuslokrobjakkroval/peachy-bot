...
// Import schemas from the main user.js file
...
// Private Server User Schema - Same as user but with guildId for server-specific data
const PrivateUserSchema = new Schema(
  {
    ...
    sponsorTransactions: { type: [SponsorTransactionSchema], default: [] },
    lastSponsorReset: { type: Date, default: Date.now },
    validation: {
    ...
    sellPetCount: { type: Number, default: 0 },
    feedCount: { type: Number, default: 0 },
    soldPets: { type: [SoldPetSchema], default: [] },
  ...
  },
  {
    ...
    // Compound index for userId + guildId combination
    index: { userId: 1, guildId: 1 },
  }
);

// Create compound unique index
PrivateUserSchema.index({ userId: 1, guildId: 1 }, { unique: true });

module.exports = model("PrivateUser", PrivateUserSchema);
