...
module.exports = class MessageCreate extends Event {
  ...
  run(message) {
    this.client.utils.getUser(message.author.id).then(user => {
      if (message.author.bot || !message.guild) return;
      const { color, emoji, language} = this.client.checkThemeUser(user);
              ...
              return ctx.sendMessage({
                embeds: [
                  this.client.embed()
                      ...
                      .setDescription(`Your registration is not yet complete. Please confirm your registration to start using the bot.`),
                ],
              });
            }
            ...
            msgPromise.then(msg => {
              ...
              collector.on('collect', async int => {
                ...
                if (int.customId === 'register') {
                  try {
                    const welcomeEmbed = this.client.embed()
                        .setColor(color.main)
                        .setTitle(`${emoji.mainLeft} 𝐖𝐄𝐋𝐂𝐎𝐌𝐄 ${emoji.mainRight}`)
                        .setThumbnail(ctx.author.displayAvatarURL({ dynamic: true, size: 1024 }))
                        .setDescription(`Welcome to the PEACHY community! Please take a moment to read and follow these guidelines...`); // Shortened for brevity.
                  ...
                  try {
                    const privacyEmbed = this.client.embed()
                        .setColor(color.main)
                        .setTitle(`${emoji.mainLeft} 𝐏𝐑𝐈𝐕𝐀𝐂𝐘 𝐏𝐎𝐋𝐈𝐂𝐘 ${emoji.mainRight}`)
                        .setDescription(`**Introduction**\nPEACHY is dedicated to ensuring your privacy and security...`); // Shortened for brevity.
