        ...
        try {
            ...
            if (interaction instanceof CommandInteraction && interaction.type === InteractionType.ApplicationCommand) {
                ...
                if (user?.verification?.timeout?.expiresAt > now) {
                    ...
                    if (minutes > 0) timeString += timeString ? ', ' : '' + `${minutes} min${minutes > 1 ? 's' : ''}`;
                    if (seconds > 0 || timeString === '') timeString += timeString ? ', ' : '' + `${seconds} sec${seconds > 1 ? 's' : ''}`;
                ...
                try {
                    ...
                    if (!interaction.guild.members.me.permissions.has(PermissionFlagsBits.SendMessages)) {
                        return await interaction.member
                            ...
                            .catch((err) => console.error(`Error sending DM to ${interaction.user.id}:`, err));
                    }
                    ...
                    const balanceCommands = ['balance', 'deposit', 'withdraw', 'multitransfer', 'transfer'];
                    const gamblingCommands = ['slots', 'blackjack', 'coinflip', 'klaklouk'];
                    const gameCommands = ['guessnumber'];

                    let logChannelId;
                    if (['admin', 'staff', 'developer', 'guild'].includes(command.category.toLowerCase())) {
                        logChannelId = this.client.config.logChannelId[9];
                    } else if (['animals', 'building'].includes(command.category.toLowerCase())) {
                    ...
                    if (channel && channel.isTextBased()) {
                        ...
                        await channel.send({ embeds: [embed] }).catch((err) => {
                            console.error(`Error sending log to channel ${logChannelId}:`, err);
                        });
                    }
                ...
                switch (interaction.customId) {
                    ...
                    case 'social_setup': {
                        ...
                        if (socialsCmd && socialsCmd.showSetupGuide) {
                            await socialsCmd.showSetupGuide(
                                ...
                                color,
                                emoji,
                                language.locales.get(language.defaultLocale)?.socialMessages?.smMessages
                            );
                    ...
                    case 'view_socials': {
                        ...
                        if (socialsCmd && socialsCmd.run) {
                            await socialsCmd.run(this.client, interaction, [], color, emoji, language);
                            if (!interaction.replied && !interaction.deferred) await interaction.deferUpdate().catch(() => {});
                        }
                        break;
                    }
                        ...
                        if (fbCmd) {
                            if (interaction.customId === 'facebook_set_name') {
                                await fbCmd.showHelpMessage(
                                    ...
                                    color,
                                    emoji,
                                    language.locales.get(language.defaultLocale)?.socialMessages?.fbMessages
                                );
                                ...
                                await fbCmd.run(this.client, interaction, [], color, emoji, language);
                            }
                            if (!interaction.replied && !interaction.deferred) await interaction.deferUpdate().catch(() => {});
                        }
                        break;
                        ...
                        if (igCmd) {
                            if (interaction.customId === 'instagram_set_name') {
                                await igCmd.showHelpMessage(
                                    ...
                                    color,
                                    emoji,
                                    language.locales.get(language.defaultLocale)?.socialMessages?.igMessages
                                );
                                ...
                                await igCmd.run(this.client, interaction, [], color, emoji, language);
                            }
                            if (!interaction.replied && !interaction.deferred) await interaction.deferUpdate().catch(() => {});
                        }
                        break;
                        ...
                        if (ttCmd) {
                            if (interaction.customId === 'tiktok_set_name') {
                                await ttCmd.showHelpMessage(
                                    ...
                                    color,
                                    emoji,
                                    language.locales.get(language.defaultLocale)?.socialMessages?.ttMessages
                                );
                                ...
                                await ttCmd.run(this.client, interaction, [], color, emoji, language);
                            }
                            if (!interaction.replied && !interaction.deferred) await interaction.deferUpdate().catch(() => {});
                        }
                        break;
                    ...
                    case 'giveaway-join': {
                        try {
                            ...
                            if (data.endTime * 1000 < Date.now()) {
                                return await this.client.utils.endGiveaway(this.client, color, emoji, interaction.message);
                            }
                            ...
                            console.error(`Error in giveaway-join handler for message ${interaction.message.id}:`, error);
                            if (!interaction.replied && !interaction.deferred) {
                                await interaction.reply({
                    ...
                    case 'leave-giveaway': {
                        try {
                            ...
                            console.error(`Error in leave-giveaway handler for message ${interaction.message.id}:`, error);
                            if (!interaction.replied && !interaction.deferred) {
                                await interaction.reply({
                                    content: 'An error occurred while processing your request.',
                                    flags: MessageFlags.Ephemeral,
                    ...
                    case 'giveaway-participants': {
                        try {
                            ...
                            console.error(`Error in giveaway-participants handler for message ${interaction.message.id}:`, error);
                            if (!interaction.replied && !interaction.deferred) {
                                await interaction.reply({
                                    content: 'An error occurred while processing your request.',
                                    flags: MessageFlags.Ephemeral,
                    ...
                    case 'giveawayshopitem-join': {
                        try {
                            ...
                            console.error(`Error in giveawayshopitem-join handler for message ${interaction.message.id}:`, error);
                            if (!interaction.replied && !interaction.deferred) {
                                await interaction.reply({
                                    content: 'An error occurred while processing your request.',
                                    flags: MessageFlags.Ephemeral,
                        ...
                        try {
                            ...
                            const participants = await Promise.all(
                                data.entered.map(async (id, index) => {
                                    try {
                                        const member =
                                            interaction.guild.members.cache.get(id) || (await interaction.guild.members.fetch(id));
                                        return `${index + 1}. <@${id}> (**1** entry)`;
                                    } catch (err) {
                                        console.error(`Unable to fetch member ${id}:`, err);
                                        return null;
                            ...
                            console.error(`Error in giveawayshopitem-participants handler for message ${interaction.message.id}:`, error);
                            if (!interaction.replied && !interaction.deferred) {
                                await interaction.reply({
                                    content: 'An error occurred while processing your request.',
                                    flags: MessageFlags.Ephemeral,
                    ...
                    case 'giveaway-join-req': {
                        try {
                            ...
                            const meetsRequirements = await this.client.utils.checkGiveawayRequirements(
                                this.client,
                                interaction.user,
                                giveaway,
                                interaction
                            ...
                            if (!meetsRequirements) {
                                return interaction.reply({
                                    content: 'You do not meet the requirements for this giveaway.',
                                    flags: MessageFlags.Ephemeral,
                                });
                            }
                        } catch (error) {
                            console.error(`Error in giveaway-join-req handler for message ${interaction.message.id}:`, error);
                            if (!interaction.replied && !interaction.deferred) {
                                await interaction.reply({
                                    content: 'An error occurred while processing your request.',
                                    flags: MessageFlags.Ephemeral,
                    ...
                    case 'claim': {
                        try {
                            ...
                            if (!user) {
                                return interaction.reply({
                                    content: 'You do not have an account. Create one to claim rewards.',
                                    flags: MessageFlags.Ephemeral,
                                });
                            }

                            const claimedCoins = Math.floor(Math.random() * (10000 - 1000 + 1)) + 1000;

                            await users.findOneAndUpdate({ userId }, { $inc: { 'balance.coin': claimedCoins, 'claim.streak': 1 } });
                            ...
                            console.error(`Error in claim handler for user ${interaction.user.id}:`, error);
                            if (!interaction.replied && !interaction.deferred) {
                                await interaction.reply({
                                    content: 'An error occurred while processing your request.',
                                    flags: MessageFlags.Ephemeral,
            ...
            } else if (interaction instanceof UserSelectMenuInteraction && interaction.type === InteractionType.MessageComponent) {
                ...
                if (interaction.customId.startsWith('avatar_select_')) {
                    ...
                    if (interaction.user.id !== requesterId) {
                        return await interaction.reply({
                            content: '❌ You cannot use this menu. Please run the command yourself.',
                            flags: MessageFlags.Ephemeral,
                        });
                    }
                    ...
                    const avatarGIF = isAnimated ? selectedUser.displayAvatarURL({ extension: 'gif', size: 4096 }) : null;
                        ...
                        .addMediaGalleryComponents((gallery) =>
                            gallery.addItems((item) =>
                                item
                                    ...
                                    .setDescription(`${selectedUser.displayName || selectedUser.username}'s Full Avatar`)
                            )
                        )
                    ...
                    try {
                        ...
                        if (!bannerURL) {
                            return await interaction.followUp({
                                content: bannerMessages?.noBannerFound || '❌ This user does not have a banner.',
                                flags: MessageFlags.Ephemeral,
                            });
                        }
                        ...
                        const bannerContainer = new ContainerBuilder()
                            ...
                            .addMediaGalleryComponents((gallery) =>
                                gallery.addItems((item) =>
                                    item.setURL(bannerURL).setDescription(`${selectedUser.displayName || selectedUser.username}'s Banner`)
                                )
                            )
                            .addSeparatorComponents((sep) => sep.setDivider(false))
                            .addSectionComponents((section) =>
                            ...
                            .addSectionComponents((section) =>
                                section
                                    ...
                                    .setButtonAccessory((button) =>
                                        button.setLabel('📦 WEBP').setStyle(ButtonStyle.Link).setURL(bannerWEBP)
                                    )
                            )
                            .addSeparatorComponents((sep) => sep.setDivider(false))
                            .addActionRowComponents((row) =>
                        ...
                        return await interaction.followUp({
                            content: bannerMessages?.error || '❌ An error occurred while fetching the banner.',
                            flags: MessageFlags.Ephemeral,
                        });
                    }
                }
            ...
            if (!interaction.replied && !interaction.deferred) {
                await interaction
                    ...
                    .catch((err) => console.error(`Error sending fallback reply for interaction ${interaction.id}:`, err));
            }
        }
    }
};
