...
module.exports = class GuildCreate extends Event {
    ...
    async run(guild) {
        ...
        const specificBlacklist = ['1370047480460214323'];

        // Check if guild ID matches the specific ID to auto-leave
        if (specificBlacklist.includes(guild.id)) {
            if (logChannel) {
                await logChannel
                    .send(
    ...
    async fetchInviterSafe(guild) {
        ...
        try {
            ...
            const entry = logs.entries.find((e) => {
                ...
                const recentEnough = typeof e.createdTimestamp === 'number' ? now - e.createdTimestamp <= LOOKBACK_MS : true; // fallback if ts missing
            ...
            if (entry && entry.executor) {
                ...
                const tag = typeof user.tag === 'string' ? user.tag : `${user.username || 'Unknown'}#${user.discriminator || '0000'}`;
    ...
    async sendGuildInfo(guild, owner, inviter) {
        ...
        if (!inviteChannel.permissionsFor(guild.members.me).has(PermissionFlagsBits.CreateInstantInvite)) {
            return channel.send('I don’t have permission to create an invite link in this channel.').catch(console.error);
        }
        ...
        try {
            ...
            const embed = this.client
                ...
                .addFields([
                    ...
                    {
                        name: 'Join Count',
                        value: (await Guild.findOne({ guildId: guild.id })).joinCount.toString(),
                        inline: true,
                    },
                    {
                        name: 'Created At',
            ...
            await channel.send('Failed to create an invite link or send guild info.').catch(console.error);
        }
    }
};
