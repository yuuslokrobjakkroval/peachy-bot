...
module.exports = class GuildCreate extends Event {
  ...
  async run(guild) {
    ...
    if (guildData.isBanned) {
      if (logChannel) {
        await logChannel.send(`Leaving guild **${guild.name}** (ID: ${guild.id}) because it is blacklisted.`).catch(console.error);
      }
    ...
    if (guildData.joinCount > 10) {
      if (logChannel) {
        await logChannel.send(`Leaving guild **${guild.name}** (ID: ${guild.id}) due to excessive joins (${guildData.joinCount}).`).catch(console.error);
      }
      await guild.leave().catch(err => console.error(`Failed to leave guild ${guild.id}:`, err));
      return; // Exit the event after leaving
    }
    ...
    try {
      ...
      const embed = this.client
          ...
          .addFields([
            ...
            { name: "Join Count", value: (await Guild.findOne({ guildId: guild.id })).joinCount.toString(), inline: true },
            {
              name: "Created At",
